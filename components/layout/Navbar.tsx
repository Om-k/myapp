...
export const Navbar = () => {
    ...
    // Default to the first segment of the URL (e.g., 'en' or 'kn')
    ...
    // Function to switch language using Next.js router
    ...
    return (
        <nav className={`fixed top-0 w-full z-[100] transition-all duration-500 ease-in-out ${
            isScrolled ? "bg-white/95 backdrop-blur-md py-3 shadow-lg" : "bg-transparent py-5"
        }`}>
            <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">

                {/* 1. LOGO */}
                <a href="/" className="flex items-center space-x-2 group shrink-0">
                    <div className="p-2 rounded-2xl transition-transform ">
                        <Image 
                            src={isScrolled ? "/Novigo.svg" : "/NovigoLight.svg"} 
                            alt="Novigo Logo"
                            width={120}
                            height={120} 
                        />
                ...
                <div className={`
                    ...
                    ${isScrolled
                        ? "bg-zinc-100/50 border-gray-300"
                    ...
                    <ul className="flex items-center space-x-1">
                        ...
                        <li className="relative ml-2 pl-2 border-l border-white/10">
                            <button
                                ...
                                className={`flex items-center space-x-2 px-5 py-2.5 rounded-full text-[11px] font-black uppercase tracking-widest transition-all transform active:scale-95 ${
                                    isScrolled ? "bg-zinc-900 text-white" : "bg-white text-zinc-950 hover:bg-zinc-100"
                                }`}
                            >
                            ...
                            {isLangOpen && (
                                <div className="absolute right-0 mt-4 w-44 bg-white rounded-2xl shadow-2xl border border-zinc-100 py-2 overflow-hidden">
                                    {LANGUAGES.map(lang => (
                                        <button
                                            key={lang.code}
                                        ...
                                        >
                                            {lang.name}
                                            {currentLocale === lang.code && <div className="w-1.5 h-1.5 bg-blue-600 rounded-full" />}
                                        </button>
                                    ))}
                                </div>
                            )}
                                                ...
                                                {link.columns?.map(col => (
                                                    <div key={col.title}>
                                                        ...
                                                        <div className="space-y-5">
                                                            {col.links.map(l => (
                                                                <a key={l.n} href="#" className="block">
